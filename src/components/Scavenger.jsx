...
const Scavenger = () => {
  ...
  return (
    <section
      ...
      style={{
        background:
          "radial-gradient(ellipse 70% 60% at 60% 50%, rgba(201,168,76,0.05), transparent)",
      }}
    >
          ...
          >
            ...
            <div className="flex-1 min-w-[220px]">
              <p className="font-body text-mist text-lg leading-relaxed">
                In 2021, Obafemi shot his first ever short film —{" "}
                <em className="text-pearl">Scavenger</em> — written and produced
                by him, and directed by Olushola Animashaun. The movie is yet to
                be released. These are behind-the-scenes moments from the
                production.
              </p>
            </div>
            ...
            <div className="flex flex-col gap-4 text-right shrink-0">
              <div>
                <p className="font-mono text-[8px] tracking-[0.3em] uppercase text-gold mb-1">
                  Written & Produced By
                </p>
                <p className="font-body text-pearl font-semibold">
                  Obafemi Oniosun
                </p>
              </div>
              <div>
              ...
              <div className="flex justify-between items-center mt-4 px-2">
                <button
                  onClick={() =>
                    setLightboxIndex(
                      (i) =>
                        (i - 1 + SCAVENGER_IMAGES.length) %
                        SCAVENGER_IMAGES.length
                    )
                  }
                ...
                <button
                  onClick={() =>
                    setLightboxIndex((i) => (i + 1) % SCAVENGER_IMAGES.length)
                  }
                  className="w-10 h-10 rounded-full border border-white/10 flex items-center justify-center text-silver hover:text-gold hover:border-gold/30 transition-all duration-300"
                >
                  →
